module.exports.getList = (inOptions, inPost, inHexo) => {
    ...
    let getElm = (list) => {
        ...
        if (inOptions.isDate && list.date != '') {
            // 修复内部逻辑处理了date数据，导致date为不规范的moment 对象，需要兼容
            // 1. TypeError: config._d.getTime is not a function
            // 2. TypeError: this.localeData(...).postformat is not a function
            // console.log(list.date._d)
            // console.log(typeof list.date)
            // console.log(typeof list.date._d)
            try {
                if (typeof list.date._d === 'string') {
        ...
        if (inOptions.isTags && list.tags) {
            ret.tags = list.tags.map(tag => ({
                name: tag.name,
                path: pathFn.join(config.root , tag.path),
            }))
        }
